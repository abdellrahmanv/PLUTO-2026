...
## Interfaces
- Commands: `CMD:RETURN`, `CMD:RESET_HOME`, `CMD:ARM:<steps>,<speed>`, `CMD:ARM2:<steps>,<speed>`, `CMD:STOP`, `CMD:DRIVE:<speed>,<steer>`
- ACKs: `ACK:RETURN`, `ACK:RETURN_COMPLETE`, `ACK:RESET_HOME`, `ACK:ARM`, `ACK:ARM_DONE`, `ACK:ARM2`, `ACK:ARM2_DONE`
...
- `send_arm2()` resets `arm2_done = False`. When the second NEMA stepper finishes motion, `ACK:ARM2_DONE` transitions `arm2_done` to `True`.
...
For the second NEMA channel, expected evidence includes `ACK:ARM2` and
`ACK:ARM2_DONE`.
...
- Unbounded ARM2 motion: Calling `send_arm2()` without safety limits may crash the second stepper. MUST NOT be called without strict bounds checks.

## Safety Notes
`send_arm()` and `send_arm2()` are low-level variable commands and must only be
called after mechanical bounds and physical limit switches are verified.
Odometry-guided `send_return()` represents a potential hazard because STM32
odometry can drift; thus, it is kept behind dry-run controls and only exercised
after thorough wheels-lifted verification.
...
- 2026-06-02: Added second NEMA channel support in STM32 firmware and Pi serial wrapper: `CMD:ARM2:<steps>,<speed>`.
