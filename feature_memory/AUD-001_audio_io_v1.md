...
selection prefers names containing camera, webcam, USB, mic, or microphone.
Playback selection prefers headphones, then speaker/USB/audio devices.
...
1. Confirm the webcam microphone exists in `arecord -l`.
...
| Wrong microphone | ALSA name did not match camera tokens | `/api/audio/status` | Set explicit code/env override later |
